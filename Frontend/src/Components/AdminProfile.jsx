...
import { 
  ...
  Camera, LayoutDashboard, MapPinned, Bell, CheckCircle2, ChevronRight, UserCircle
} from "lucide-react";
...
const AdminProfile = () => {
  ...
  // ✅ Updated Menu Items with "All Agents" and "Round Trip"
  const menuItems = [
    ...
    { name: "All Agents", path: "/allagents", icon: <Handshake size={18}/> }, // ✅ Vapas Add Kar Diya
      ...
      <div className="flex">
        ...
        <aside className={`fixed md:sticky top-0 h-screen w-72 bg-white border-r border-gray-100 transition-all z-50 ${sidebarOpen ? "left-0" : "-left-72 md:left-0"}`}>
          <div className="p-8 h-full flex flex-col">
            <h2 className="text-2xl font-black italic tracking-tighter uppercase mb-12">All <span className="text-orange-600 italic underline decoration-4 underline-offset-4">India</span></h2>
            
            <nav className="flex-1 space-y-1 overflow-y-auto pr-2 custom-scrollbar">
              {menuItems.map((item, index) => (
                <Link key={index} to={item.path} className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all group ${location.pathname === item.path ? "bg-gray-950 text-white shadow-xl shadow-gray-900/20" : "text-gray-500 hover:bg-gray-50 hover:text-gray-950"}`}>
        ...
        <main className="flex-1 p-6 md:p-12">
          <header className="flex justify-between items-center mb-12">
             ...
             <button onClick={() => setSidebarOpen(true)} className="md:hidden bg-white p-3 rounded-xl shadow-sm border border-gray-100 text-orange-600"><Menu size={24}/></button>
          </header>
