...
const pricingData = {
  "Dzire Car": { extraHr: 200, extraKm: 11, rates: { "6-60": 1600, "6-80": 1800, "8-80": 2000, "8-100": 2200, "10-80": 2200, "10-100": 2300, "12-80": 2300, "12-100": 2500 } },
  "Zest": { extraHr: 200, extraKm: 11, rates: { "6-80": 1700, "6-100": 1900, "8-80": 2000, "8-100": 2200, "10-80": 2200, "10-100": 2300, "12-80": 2300, "12-100": 2500 } },
  "Ertiga": { extraHr: 200, extraKm: 14, rates: { "6-80": 2200, "6-100": 2200, "8-80": 2400, "8-100": 2600, "10-80": 2600, "10-100": 2800, "12-80": 2900, "12-100": 3300 } },
  "Innova": { extraHr: 200, extraKm: 15, rates: { "8-80": 2500, "8-100": 2800, "10-80": 2800, "10-100": 3000, "12-80": 3200, "12-100": 3500 } },
  "Inova Crista": { extraHr: 200, extraKm: 17, rates: { "8-80": 2800, "8-100": 2900, "10-80": 3000, "10-100": 3200, "12-80": 3500, "12-100": 3800 } },
  "Tavera 10": { extraHr: 200, extraKm: 14, rates: { "8-80": 2300, "8-100": 2500, "10-80": 2500, "10-100": 2800, "12-80": 2800, "12-100": 3000 } },
       ...
       <div className="bg-gradient-to-r from-orange-600 to-amber-500 py-12 px-6 text-center text-white shadow-lg">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="mt-3 text-md md:text-lg font-medium text-orange-50 italic">
            "Booking For Local City"
          </p>
        </motion.div>
      </div>
      <div className="max-w-4xl mx-auto px-4 mt-6 space-y-6">
        {trips.map((trip, index) => (
          <div key={trip.id} className="bg-white rounded-3xl shadow-sm border border-slate-200 p-5 relative">
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <div className="flex items-center gap-2 w-full border-2 border-slate-100 p-3 rounded-xl bg-slate-50">
                  <MapPin size={16} className="text-orange-500" />
                  <input 
                    type="text" 
                    value={trip.startPoint} 
                    readOnly 
                    className="bg-transparent outline-none font-bold text-sm text-slate-600 w-full"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-sm outline-none focus:border-orange-500" onChange={(e) => updateTripField(trip.id, 'time', e.target.value)}>
                    ...
                    {[6, 8, 10, 12].map(h => <option key={h} value={h}>{h} Hr</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-sm outline-none focus:border-orange-500" onChange={(e) => updateTripField(trip.id, 'runningKm', e.target.value)}>
                    <option value="">Select</option>
                    {[80, 100, 150].map(k => <option key={k} value={k}>{k} Km</option>)}
                  </select>
                </div>
              </div>
